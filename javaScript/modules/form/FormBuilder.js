...
export default class FormBuilder {
  ...
  addInput(
    ...
    const input = createHTMLElement("input", classList, {
      id,
      type,
      name,
      required,
  ...
  addButton(classList = [], id = "", type = "", textContent = "") {
    const button = createHTMLElement("button", classList, {
      id,
      type,
      textContent,
    });
  ...
  addSelect(classList = [], required = false, options = []) {
    ...
    for (const option of options) {
      ...
      const optionElement = createHTMLElement("option", [], {
        value,
        textContent,
      });
  ...
  addTextarea(classList = [], type = "", required = false, placeholder = "") {
    const textarea = createHTMLElement("textarea", classList, {
      name,
      required,
      placeholder,
    });
